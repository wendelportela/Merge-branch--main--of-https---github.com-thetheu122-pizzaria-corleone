...
export default function ClienteAdm() {
  ...
  async function UpdateStatus(novoStatus) {
    try {
      ...
      toast.success(`Pedido ${data.id} atualizado para ${novoStatus}`);
    } catch (error) {
      toast.error(error.message);
    }
  }

  async function UpdateStatusParte1(novoStatus) {
    try {
      let data = {
        situacao: novoStatus,
        id: idp,
      ...
      console.log(data.id)
      const r = await axios.put(`${API_URL}/pedido/rastreamento/empreparo/${data.id}`);
  ...
  {item.status.includes('Em preparo') && (
